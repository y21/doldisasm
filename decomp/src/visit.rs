...
pub struct VisitorCx<'analysis, 'a, 'b> {
    pub analysis: &'analysis LocalGenerationAnalysis<'b>,
...
pub fn visit_and_join_paths<V: SuccessorsVisitor>(
    ...
    if let Some(common_merge_inst) = common_merge_inst {
        ...
        next_state
            ...
            .register_iter()
            .zip(iter::zip(
                true_res.state.registers.register_iter(),
                false_res.state.registers.register_iter(),
            ))
            .for_each(
...
pub fn visit_path<V: SuccessorsVisitor>(
    ...
    VisitPathResult { state: block_state }
}
